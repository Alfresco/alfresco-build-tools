...
def _resolve_action_path(config) -> Path:
    ...
    return candidates[0] / ACTION_FILE


@pytest.fixture
def action_script_path(request) -> Path:
    return _resolve_action_path(request.config)


@pytest.fixture
def action_module(request) -> ModuleType:
    config = request.config
    action_path = _resolve_action_path(config)
